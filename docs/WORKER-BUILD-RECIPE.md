...
**Point-in-time comparison, written 2026-08-17** — at the time, the Flash
`@Endpoint` Runner (`app/runner_endpoint.py`) was the known-working reference
and the Docker path was untested. That has since flipped: the Docker Runner
is now the live production path and Flash's Runner has been retired (see
[ARCHITECTURE.md](ARCHITECTURE.md)'s "GPU Runner" section). Left as-is below
for its comparison detail; don't read "known-working"/"NOT settled yet" as
current.
...
| **System packages (apt)** | none currently required | `libgl1`, `libglib2.0-0` (already in `runner_endpoint.py`'s `system_dependencies=`; CLAUDE.md also recommends `libgthread-2.0`/`libxcb` proactively for future image-generation work, but the current Docker image builds and runs successfully without them) |
...
| **Persistent storage** | `mflux-orchestrator` NetworkVolume, mounted at `/runpod-volume`, holding `mflux-models.sqlite` (live-confirmed to survive worker scale-to-zero and redeploys) | A per-series ephemeral NetworkVolume (sized via `app/runpod_volumes.py::size_for_series()` — upstream HF repo size + `VOLUME_HEADROOM_GB`), holding downloaded source weights + in-progress build artifacts, deleted once the series is complete |
| **Secrets / env** | `HF_TOKEN` (RunPod Secret), `HF_ORG=mflux-community`, `REPORT_DB_PATH=/runpod-volume/mflux-models.sqlite` | `HF_TOKEN` (RunPod Secret), `HF_XET_HIGH_PERFORMANCE=1`, ~~`ORCHESTRATOR_BASE_URL`~~ **legacy, unused** — supported the old direct-HTTP callback, superseded by the DO Spaces outbox (`app/outbox.py`); current Docker Runner env is `HF_TOKEN` + `DO_SPACES_*` (see `app/orchestrator_endpoint.py`'s `RUNNER_ENDPOINT_ID` comment) |
